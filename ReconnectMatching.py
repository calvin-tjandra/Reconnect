...
def Calculate_Likelihood_Ratios(match_List, child_Homo_Count, num_SNP_Pos, pMATCHRAND):
	...
	if match_List[parent_Idx] >= MINMATCH and num_SNP_Pos - match_List[parent_Idx] <= MAXMATCH:
	    #pGREAL = dec.Decimal(pMATCHREAL ** match_List[parent_Idx] * (1 - pMATCHREAL) ** child_Homo_Count) 
	    #pGRAND = dec.Decimal(pMATCHRAND ** match_List[parent_Idx] * (1 - pMATCHRAND) ** child_Homo_Count)
	    #likelihood_List[parent_Idx] = dec.Decimal(pGREAL/pGRAND)
